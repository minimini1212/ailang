...
전체를 `permitAll` 로 열어 두었다 — 그래야 학년만 주고 보는 쪽이 로그인 없이 된다.

🔄 **2026-09-15: 여기서 나던 500 두 가지를 고쳤다.** 둘 다 원인은 학생 쪽(로그인을 안 했거나
값을 잘못 보냈거나)인데 응답은 「서버 내부 에러」였다 — 학생은 자기가 할 수 있는 일이
있다는 것을 알 수 없었다.

| 부르는 방식 | 전 | 후 |
| --- | --- | --- |
| 비로그인 + `grade` 없음 | 🔴 `500` (`userDetails` 가 null 인데 `.getEmail()` 호출) | **`401`** 「로그인이 필요한 요청입니다」 |
| `grade=중1` 처럼 어휘 밖의 값 | 🔴 `500` (`Grade.valueOf` 의 예외를 받는 핸들러가 없었다) | **`400`** 「학년 값이 올바르지 않습니다. (ELEM_3 ~ HIGH_1)」 |

⚠️ **보안 설정으로는 못 고친다.** 같은 경로가 파라미터 유무로 두 가지 일을 하고, 그중
하나는 정말로 비로그인에 열려 있어야 한다. 그래서 인증 여부를 **컨트롤러에서** 본다.
🔬 검사: `SecurityConfigTest` · `GradeFromTest`
