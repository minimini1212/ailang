...
| 필드 | 필수 | 설명 |
| --- | --- | --- |
| `chapterId` | ✓ | 🔴 **문제가 이 챕터에 속하는지 서버가 검증하지 않는다** |
| `userAnswer` | ✓ | 객관식은 보기 번호(`"1"`~`"4"`), 단답형은 답 문자열 |
| `selfJudge` | 단답형만 | 🔴 **클라이언트가 보내는 정답 판정.** 빠지면 **오답으로 기록** |

**채점**
- 객관식 → 서버가 `normalizeAnswer` 로 정규화 후 비교
  🔴 **이 정규화는 지금 서로 다른 답을 같은 것으로 만든다.** 아는 표기(`\frac`·원문자·공백)는
  잘 흡수하지만, **모르는 LaTeX 명령어는 지워 버려서** `$\sqrt{2}$` 와 `2` 가 같아지고
  `$\pi$` 는 빈 문자열이 된다(= `"$"` 한 글자로 정답 처리). 재현 결과와 대응:
  [`review/oh-my-claudecode/REVIEW_2026-08-27_full-codebase.md`](review/oh-my-claudecode/REVIEW_2026-08-27_full-codebase.md) §1
- 단답형 → **`selfJudge` 를 그대로 믿는다**

🔴 이 설계의 문제와 규율: [`rules/grading-and-difficulty.md`](rules/grading-and-difficulty.md)
...
### GET `/{problemId}/answer` — 🔴 지금 상태로는 쓰면 안 된다

```json
{ "answer": "2", "explanation": "…" }
```

단답형 자가채점 직전에 정답을 보여주려고 만들었다. 그런데 **`problemType` 도, 「이 학생이
이 문제를 이미 풀었나」도 확인하지 않는다.** 로그인만 하면 어떤 문제의 정답이든,
객관식이든, 제출 전이든 받을 수 있다.
