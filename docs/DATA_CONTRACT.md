...
| `SUPABASE_PROJECT_URL` `SUPABASE_PUBLISHABLE_SECRET_KEY` | FastAPI | ⬜ 읽는 코드가 없다 (Phase 3). 🔄 2026-09-01 에 **선택 필드로** 바뀌어, 비어 있어도 서버가 뜬다 |
...
| `APP_FRONTEND_ORIGIN` | Spring | 🔄 **2026-09-15 신설.** CORS 허용 출처 + 로그인 후 돌아갈 주소. 쉼표로 여러 개 |
| `APP_BACKEND_ORIGIN` | Spring | 🔄 **2026-09-15 신설.** 백엔드 자신의 구글 콜백 주소. ⚠️ 구글 콘솔의 「승인된 리디렉션 URI」와 글자까지 같아야 한다 |
...
| `JPA_SHOW_SQL` | Spring | 🔴 운영에서 켜지 않는다 — SQL 전문에 학생 이메일과 답안이 실린다. 기본 `false` |
