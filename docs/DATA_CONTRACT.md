...
🔴 **그리고 본문 안의 보기 표기가 «두 가지» 다.** 2026-09-16 에 객관식 500건을 전수로
세어 알았다.
...
⚠️ **한쪽만 읽는 파서를 쓰면 20문제가 「보기 없음」이 된다** — 실제로 2026-09-16 까지
그랬고, 그 문제들은 화면에서 **고를 것이 안 나와 넘어갈 수도 없었다**.
정본 파서는 화면 저장소의 `src/lib/multipleChoice.ts` 한 곳이다.

🔴 **id 2919 는 보기가 본문보다 «앞» 에 있다** — OCR 이 페이지를 그 순서로 읽었다.
파서로는 못 고친다(어디까지가 보기인지 추측해야 한다). 자료 문제다.
