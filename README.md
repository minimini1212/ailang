...
> **상태:** 🟡 **Phase 2 완료 · Phase 3(RAG) 미착수** · 2026-08-27 기준
>
> 🔴 **정답 판정을 클라이언트가 하고 있다.** 단답형은 프론트가 보낸 「맞음/틀림」을 서버가
> 그대로 저장하고, 정답 공개 API 는 객관식 정답까지 제출 전에 내려준다.
> 이 상태에서는 난이도 조정 알고리즘이 성립하지 않는다.
> → [docs/rules/grading-and-difficulty.md](docs/rules/grading-and-difficulty.md)
...
| 챕터 목록 (학년별 + 내 통계) | ✅ | ✅ |
| 진단 테스트 20문제 | ✅ | ✅ |
| 답안 제출 · 통계 갱신 · 난이도 재계산 | 🟡 **조작 가능** | (진단에서만) |
...
| 4 | `normalizeAnswer` 충돌 | `√2` 와 `2` 가 같아진다. **진단 테스트 채점에 지금 쓰인다** |
